...
public class ConfigAop {
     ...
     * 定义有一个切入点，范围为web包下的类
     */
    @Pointcut("execution(public * com.aiwsport.web.controller.*.*(..))")
            ...
            if (methodAnnotation instanceof ParamObjVerifys) {// 是否是方法校验类型注解，排除其他类型注解
                ...
                for (ParamObjVerify paramObjVerify : ParamObjVerifyList) {
                    ...
                    try {
                        ...
                        throw new DrawServerException(DrawServerExceptionFactor.CONFIG_ERROR);
                    } catch (IllegalAccessException e) {
                        throw new DrawServerException(DrawServerExceptionFactor.INTERNAL_ERROR);
                    }
                }
            }
        }
    ...
    private void verify(ParamVerify paramVerify, Object value, String fieldName) {
        if (paramVerify == null) {
            throw new DrawServerException(DrawServerExceptionFactor.CONFIG_PARAM_TYPE_MISMATCH);
        }
        String paramValue = value == null ? null : String.valueOf(value);

        if (paramVerify.isNotNull()) {
            if (value == null) {
                throw new DrawServerException(DrawServerExceptionFactor.PARAM_VERIFY_FAIL, fieldName+"不能为空");
            }
        }
        ...
        if (StringUtils.isNotBlank(paramValue)) {
            if (paramVerify.maxLen() != Integer.MAX_VALUE) {
                if (paramValue.length() > paramVerify.maxLen()) {
                    throw new DrawServerException(DrawServerExceptionFactor.PARAM_VERIFY_FAIL, fieldName+"长度超过限制");
                }
            }
            if (paramVerify.minLen() != -1) {
                if (paramValue.length() < paramVerify.minLen()) {
                    throw new DrawServerException(DrawServerExceptionFactor.PARAM_VERIFY_FAIL, fieldName+"长度小于超过限制");
                }
            }
            if (paramVerify.isIp()) {
                if (!RegexUtil.isIp(paramValue)) {
                    throw new DrawServerException(DrawServerExceptionFactor.PARAM_VERIFY_FAIL, fieldName+"不是有效的ip地址");
                }
            }
            ...
            if (paramVerify.isNumber()) {
                if (!RegexUtil.isNumber(paramValue)) {
                    throw new DrawServerException(DrawServerExceptionFactor.PARAM_VERIFY_FAIL, fieldName+"不是正数");
                }
            }
            if (paramVerify.isHostOrIp()) {
                if (!RegexUtil.isIp(paramValue) && !RegexUtil.isHost(paramValue)) {
                    throw new DrawServerException(DrawServerExceptionFactor.PARAM_VERIFY_FAIL, fieldName+"不是有效的ip或域名");
                }
            }
            if (paramVerify.isBoolean()) {
                if (!RegexUtil.isBoolean(paramValue)) {
                    throw new DrawServerException(DrawServerExceptionFactor.PARAM_VERIFY_FAIL, fieldName+"不是正确的boolean值");
                }
            }
            if (paramVerify.isNotChinese()) {
                if (RegexUtil.isChinese(paramValue)) {
                    throw new DrawServerException(DrawServerExceptionFactor.PARAM_VERIFY_FAIL, fieldName+"不可包含汉字");
                }
            }
            if (paramVerify.isNull()) {
                throw new DrawServerException(DrawServerExceptionFactor.PARAM_VERIFY_FAIL, fieldName+"不可修改");
            }
        }
    }
}
